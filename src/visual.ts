...
} from "./utils/CriticalBarStyle";
import type { CriticalBarStyle, CriticalStatusMarkerDescriptor, CriticalTaskLike } from "./utils/CriticalBarStyle";
...
type StatusMarkerBatch = {
    x1: number;
    x2: number;
    y: number;
};

type MilestoneStatusMarkerBatch = {
    x: number;
    y: number;
    size: number;
};
...
[data-owner="${this.instanceId}"] svg .table-timeline-header-divider,
...
`
    ...
    private getCurrentBarDateMode(): CurrentBarDateMode {
        return normalizeCurrentBarDateMode(this.settings?.taskBars?.currentBarDateMode?.value?.value);
    }

    private getCriticalBarStyle(): CriticalBarStyle {
        return normalizeCriticalBarStyle(this.settings?.criticalPath?.criticalBarStyle?.value?.value);
    }
    ...
    private getTaskBarSortDate(task: Task): Date | null {
        return this.getTaskBarGeometry(task).sortDate;
    }

    private getTaskBarLabelStart(task: Task): Date | null {
        return this.getTaskBarGeometry(task).labelStartDate;
    }

    private getTaskBarLabelFinish(task: Task): Date | null {
        return this.getTaskBarGeometry(task).labelFinishDate;
    }

    private getTaskBarDurationSegment(task: Task): TaskBarSegment | null {
        const geometry = this.getTaskBarGeometry(task);
        return geometry.segments.find(segment => segment.kind === "scheduled") ?? geometry.segments[0] ?? null;
    }

    private getScheduleStart(task: Task): Date | null {
        return getScheduleStart(task);
    }

    private getScheduleFinish(task: Task): Date | null {
        return getScheduleFinish(task);
    }

    private getVisualMilestoneDate(task: Task): Date | null {
        return this.getTaskBarGeometry(task).milestoneDate;
    }

    private hasValidVisualDates(task: Task): boolean {
        const geometry = this.getTaskBarGeometry(task);
        if (geometry.isMilestone) {
            return geometry.milestoneDate instanceof Date && !isNaN(geometry.milestoneDate.getTime());
        }
    ...
    private ensureTaskSortCache(signature: string): void {
        ...
        const sortedByStartDate = this.allTasksData
            .filter(task => {
                const s = this.getTaskBarSortDate(task);
    ...
    private getFallbackProgressLineReference(reference: ProgressLineReference): ProgressLineReference {
    ...
    private setProgressLineReferenceSettingValue(reference: ProgressLineReference): void {
        ...
        this.settings.progressLine.referenceFinish.value = {
            value: reference,
    ...
    private toggleProgressLineDisplay(): void {
        const nextVisible = !(this.settings?.progressLine?.show?.value ?? false);
        this.setProgressLineVisible(nextVisible);
    }

    private setProgressLineVisible(visible: boolean): void {
        if (!this.settings?.progressLine?.show) {
            return;
        }
    ...
    private setProgressLineReference(reference: ProgressLineReference): void {
        if (reference !== "baselineFinish" && reference !== "previousUpdateFinish") {
            return;
        }
    ...
    private createMarginResizer(): void {
        ...
        this.marginResizer = wrapper.append("div")
            ...
            .attr("aria-label", "Resize table and timeline divider")
            .style("position", "absolute")
            .style("top", "0")
            .style("bottom", "0")
            .style("height", "auto")
        ...
        this.marginResizer.append("div")
            ...
            .style("background-color", this.highContrastMode ? this.getForegroundColor() : this.getHeaderLegendBorderColor())
            .style("opacity", "1")
            .style("pointer-events", "none");
    ...
    private updateMarginResizerPosition(): void {
        ...
        const effectiveMargin = this.getEffectiveLeftMargin();
        const headerBandTop = this.getHeaderBandMetrics().top;
        const footerOffset = Math.max(0, wrapperRect.bottom - containerRect.bottom);
        const dividerColor = this.highContrastMode
            ? this.getForegroundColor()
            : this.getHeaderLegendBorderColor();
        ...
        this.marginResizer
            ...
            .style("top", `${Math.max(0, Math.round(headerBandTop))}px`)
            .style("bottom", `${Math.round(footerOffset)}px`)
            .style("height", "auto")
            .attr("aria-valuenow", Math.round(effectiveMargin).toString());

        this.marginResizer.select<HTMLDivElement>(".margin-resizer-line")
            .style("background-color", dividerColor);
    }
    ...
    private handleMarginDragUpdate(newLeftMargin: number): void {
        ...
        this.drawProgressLine(renderableTasks, this.xScale, this.yScale, chartWidth, chartHeight, true);
    ...
    private drawHeaderDivider(viewportWidth: number): void {
        ...
        lineSelection.enter()
            ...
            .attr("x1", this.snapLineCoord(dividerInsetX))
            .attr("x2", d => this.snapLineCoord(Math.max(dividerInsetX, d - dividerInsetX)))
            .attr("y1", dividerY)
            .attr("y2", dividerY);
    }
    ...
    private setupTimeBasedSVGAndScales(
        ...
        tasksToShow.forEach(task => {
            ...
            const geometry = this.getTaskBarGeometry(task);
            if (geometry.isMilestone) {
                if (geometry.milestoneDate && !isNaN(geometry.milestoneDate.getTime())) {
                    allTimestamps.push(geometry.milestoneDate.getTime());
                }
            } else {
                for (const segment of geometry.segments) {
                    if (!isNaN(segment.start.getTime())) {
                        allTimestamps.push(segment.start.getTime());
                    }
                    if (!isNaN(segment.finish.getTime())) {
                        allTimestamps.push(segment.finish.getTime());
                    }
                }
            }
    ...
    private getTaskAtCanvasPoint(x: number, y: number): Task | null {
        ...
        for (const task of visibleTasks) {
            ...
            const geometry = this.getTaskBarGeometry(task);
            if (geometry.isMilestone) {
                const milestoneDate = geometry.milestoneDate;
                ...
                for (const segment of geometry.segments) {
                    const taskX = this.xScale(segment.start);
                    const taskWidth = this.xScale(segment.finish) - taskX;
    ...
    private drawVisualElements(
        ...
        this.drawProgressLine(renderableTasks, xScale, yScale, chartWidth, chartHeight);
    ...
    private drawTasks(
        ...
        const getTaskFillColor = (d: Task, fallbackColor: string, applyCriticalFormat: boolean = true): string =>
            this.getSemanticTaskFillColor(d, fallbackColor, criticalColor, nearCriticalColor, applyCriticalFormat);
        ...
        allTaskGroups.filter((d: Task) =>
            ...
            .each(function (d: Task) {
                ...
                const labelFinish = geometry.labelFinishDate;
                const segmentCount = geometry.segments.length;

                for (const segment of geometry.segments) {
                    const startX = self.snapRectCoord(xScale(segment.start));
                    const finishX = self.snapRectCoord(xScale(segment.finish));
                    const barWidth = Math.max(1, finishX - startX);
                    const barRadius = Math.min(taskBarCornerRadius, barWidth / 2, barHeight / 2);
                    const applyCriticalFormat = self.shouldApplyCriticalFormatToSegment(segment);
                    const baseFillColor = getTaskFillColor(d, taskColor, applyCriticalFormat);
                    ...
                    d3.select(this).append("rect")
                        .attr("class", (d: Task) => {
                            const segmentClass = `task-bar-${segment.kind}`;
                            if (applyCriticalFormat && d.isCritical) return `task-bar ${segmentClass} critical`;
                            if (applyCriticalFormat && d.isNearCritical) return `task-bar ${segmentClass} near-critical`;
                            return `task-bar ${segmentClass} normal`;
                        })
                        .attr("role", "button")
                        .attr("aria-label", (d: Task) => {
                            ...
                            const splitText = segmentCount > 1 ? `, ${segment.kind} segment` : "";
                            return `${d.name}, ${statusText} task${splitText}${lookAheadText}, Start: ${self.formatDate(labelStart)}, Finish: ${self.formatDate(labelFinish)}${selectedText}. Press Enter or Space to select.`;
                        })
                        .attr("tabindex", segment.kind === "started" && segmentCount > 1 ? -1 : 0)
                    ...
                    const overlay = self.getBeforeDataDateOverlay(segment.start, segment.finish, startX, finishX, barRadius);
                    if (overlay) {
                    ...
                    const markerStyle = self.getCriticalStatusMarkerStyle(d, criticalColor, nearCriticalColor, false, applyCriticalFormat);
                    if (markerStyle) {
                        const markerInset = Math.min(
                            Math.max(0, barWidth / 2 - 0.5),
                            Math.max(0, Math.min(barRadius, markerStyle.thickness))
                        );
                        const markerX1 = startX + markerInset;
                        const markerX2 = finishX - markerInset;

                        d3.select(this)
                            .append("line")
                            .attr("class", `critical-status-marker ${markerStyle.status === "critical" ? "critical" : "near-critical"}`)
                            .attr("aria-hidden", "true")
                            .attr("x1", markerX1)
                            .attr("x2", markerX2)
                            .attr("y1", barYOffset + markerStyle.thickness / 2)
                            .attr("y2", barYOffset + markerStyle.thickness / 2)
                            .style("stroke", markerStyle.color)
                            .style("stroke-width", markerStyle.thickness)
                            .style("stroke-linecap", "round")
                            .style("stroke-dasharray", markerStyle.dashed ? markerStyle.dashArray : "none")
                            .style("pointer-events", "none");
                    }
                }
            });
        ...
        allTaskGroups.filter((d: Task) =>
            ...
            .each(function (d: Task) {
                ...
                if (mDate instanceof Date && !isNaN(mDate.getTime())) {
                    ...
                    const markerStyle = self.getCriticalStatusMarkerStyle(d, criticalColor, nearCriticalColor, true);
                    if (markerStyle) {
                        d3.select(this).append("path")
                            .attr("class", `critical-status-ring ${markerStyle.status === "critical" ? "critical" : "near-critical"}`)
                            .attr("aria-hidden", "true")
                            .attr("transform", (d: Task) => {
                                const x = xScale(mDate);
                                if (isNaN(x)) console.warn(`Invalid X position for milestone ${d.internalId}`);
                            ...
                            .attr("d", () => getMilestonePath(milestoneShape, milestoneSize + markerStyle.thickness))
                            .style("fill", "none")
                            .style("stroke", markerStyle.color)
                            .style("stroke-width", markerStyle.thickness)
        ...
        if (showDuration) {
            ...
            allTaskGroups.filter((d: Task) =>
                ...
                .style("pointer-events", "none")
                .attr("x", (d: Task): number | null => {
                    const segment = self.getTaskBarDurationSegment(d);
                    if (!segment) return null;
                    const startX = xScale(segment.start);
                    const finishX = xScale(segment.finish);
    ...
    private getHeaderBandMetrics(): {
        ...
        return {
            ...
            columnY: this.snapTextCoord(top + bandHeight / 2),
    ...
    private reflowHeaderLineLabels(
        ...
        const labelGroups = Array.from(headerLayer.selectAll<SVGGElement, unknown>(
            ".data-date-label-group, .look-ahead-window-label-group, .progress-line-label-group, .previous-update-end-label-group, .baseline-end-label-group, .project-end-label-group"
        ).nodes());
    ...
    private drawColumnHeaders(currentLeftMargin: number): void {
        ...
        const bandInsetX = 8;
        const bandWidth = Math.max(1, viewportWidth - bandInsetX * 2);
        ...
        const tableTimelineDividerX = this.snapLineCoord(currentLeftMargin);
        colHeaderLayer.selectAll<SVGLineElement, number>(".table-timeline-header-divider")
            .data([tableTimelineDividerX])
            .join(
                enter => enter.append("line").attr("class", "table-timeline-header-divider"),
                update => update,
                exit => exit.remove()
            )
            .attr("x1", d => d)
            .attr("x2", d => d)
            .attr("y1", this.snapLineCoord(bandMetrics.top))
            .attr("y2", this.snapLineCoord(bandMetrics.top + bandMetrics.height))
            .style("stroke", headerPalette.divider)
            .style("stroke-width", "1px")
            .style("pointer-events", "none");
    }
    ...
    private drawTasksCanvas(
        ...
        const statusMarkerBatches = new Map<string, StatusMarkerBatch[]>();
        const milestoneStatusMarkerBatches = new Map<string, MilestoneStatusMarkerBatch[]>();
        ...
        for (const task of tasks) {
            ...
            if (task.type === 'TT_Mile' || task.type === 'TT_FinMile') {
                ...
                if (mDate) {
                    ...
                    const markerStyle = this.getCriticalStatusMarkerStyle(task, criticalColor, nearCriticalColor, true);
                    if (markerStyle) {
                        const markerKey = this.getCriticalStatusMarkerBatchKey(markerStyle);
                        if (!milestoneStatusMarkerBatches.has(markerKey)) milestoneStatusMarkerBatches.set(markerKey, []);
                        milestoneStatusMarkerBatches.get(markerKey)!.push({
                            x,
                            y,
                            size: milestoneSize + markerStyle.thickness
                        });
                    }
                }
            } else {
                const geometry = this.getTaskBarGeometry(task);
                for (const segment of geometry.segments) {
                    const startX = this.snapRectCoord(xScale(segment.start));
                    const finishX = this.snapRectCoord(xScale(segment.finish));
                    const x = startX;
                    ...
                    const semanticFill = this.getSemanticTaskFillColor(task, taskColor, criticalColor, nearCriticalColor, applyCriticalFormat);
                    const renderStyle = this.getTaskRenderStyle(
                        task,
                        semanticFill,
                        w,
                    ...
                    const overlay = this.getBeforeDataDateOverlay(segment.start, segment.finish, startX, finishX, r);
                    if (overlay) {
                    ...
                    const markerStyle = this.getCriticalStatusMarkerStyle(task, criticalColor, nearCriticalColor, false, applyCriticalFormat);
                    if (markerStyle) {
                        const markerKey = this.getCriticalStatusMarkerBatchKey(markerStyle);
                        const markerInset = Math.min(
                            Math.max(0, w / 2 - 0.5),
                            Math.max(0, Math.min(r, markerStyle.thickness))
                        );
                        if (!statusMarkerBatches.has(markerKey)) statusMarkerBatches.set(markerKey, []);
                        statusMarkerBatches.get(markerKey)!.push({
                            x1: x + markerInset,
                            x2: x + w - markerInset,
                            y: y + markerStyle.thickness / 2
                        });
                    }
                }
            }
        ...
        statusMarkerBatches.forEach((batch, markerKey) => {
            const [stroke, widthStr, dashStyle] = markerKey.split('|');
            const markerWidth = parseFloat(widthStr);
            ...
            ctx.lineCap = 'round';
            ctx.lineJoin = 'round';
            ctx.setLineDash(dashStyle === "dashed"
                ? [Math.max(3, markerWidth * 2.4), Math.max(2, markerWidth * 1.5)]
                : []);
            ctx.beginPath();
            for (const marker of batch) {
                ctx.moveTo(marker.x1, marker.y);
                ctx.lineTo(marker.x2, marker.y);
            }
            ctx.stroke();
            ctx.restore();
        });
        ...
        milestoneStatusMarkerBatches.forEach((batch, markerKey) => {
            const [stroke, widthStr, dashStyle] = markerKey.split('|');
            const markerWidth = parseFloat(widthStr);
            ...
            for (const marker of batch) {
                ...
                switch (milestoneShape) {
                    case "circle": {
                        ctx.arc(marker.x, marker.y, half, 0, Math.PI * 2);
                        break;
                    }
                    case "square": {
                        ctx.rect(marker.x - half, marker.y - half, marker.size, marker.size);
                        break;
                    }
                    case "diamond":
                    default: {
                        ctx.moveTo(marker.x, marker.y - half);
                        ctx.lineTo(marker.x + half, marker.y);
                        ctx.lineTo(marker.x, marker.y + half);
                        ctx.lineTo(marker.x - half, marker.y);
                        ctx.closePath();
                        break;
                    }
        ...
        taskGroups.append("rect")
            ...
            .attr("aria-label", (d: Task) => {
                ...
                if (d.type === 'TT_Mile' || d.type === 'TT_FinMile') {
                    ...
                    return `${d.name}, ${statusText} task, Start: ${this.formatDate(this.getTaskBarLabelStart(d))}, Finish: ${this.formatDate(this.getTaskBarLabelFinish(d))}${selectedText}. Press Enter or Space to select.`;
                }
            })
            .attr("tabindex", 0)
    ...
    private getProgressLineReferenceSetting(): ProgressLineReference {
        const rawValue = this.settings?.progressLine?.referenceFinish?.value?.value;
    ...
    private getProgressLinePoints(
        ...
        const taskHeight = this.settings.taskBars.taskHeight.value;
        const includeWbsGroups = this.settings?.progressLine?.includeWbsGroups?.value ?? true;
        const wbsGroupingEnabled = this.wbsDataExists && this.settings?.wbsGrouping?.enableWbsGrouping?.value;

        if (wbsGroupingEnabled && includeWbsGroups) {
            for (const group of this.wbsGroups) {
                if (group.yOrder === undefined) {
                    continue;
                }
                if (group.yOrder < this.viewportStartIndex || group.yOrder > this.viewportEndIndex) {
                    continue;
                }
                rowItems.push({
                    id: `wbs:${group.id}`,
    ...
    private drawProgressLineSvg(
        ...
        const progressLineGroup = this.taskLayer.append("g")
    ...
    private drawProgressLineHeaderLabel(
        ...
        if (!this.headerGridLayer?.node() || !this.settings?.progressLine?.showLabel?.value || !isValidProgressDate(this.dataDate)) {
            return;
        }

        const rawDataDateX = xScale(this.dataDate);
        if (!Number.isFinite(rawDataDateX)) {
            return;
        }
        ...
        const textElement = labelGroup.append("text")
            .attr("class", "progress-line-label")
            .attr("x", labelX)
        ...
        this.fitSvgTextToWidth(
            textElement as Selection<SVGTextElement, unknown, null, undefined>,
            labelText,
            Math.max(80, chartWidth - labelX - this.HEADER_LINE_LABEL_EDGE_PADDING)
        );
        ...
        labelGroup.insert("rect", ".progress-line-label")
            ...
            .attr("ry", 4)
            .style("fill", this.highContrastMode ? this.highContrastBackground : "#FFFFFF")
            .style("fill-opacity", this.highContrastMode ? 1 : 0.86)
            .style("stroke", lineColor)
            .style("stroke-width", "1");
    }
    ...
    private drawProjectEndLine(
        ...
        const latestFinishDate = settings.show.value
            ? this.getLatestFinishDate(allTasks, (t: Task) => this.getTaskBarLabelFinish(t))
            : null;
    ...
    private restoreScrollPosition(totalSvgHeight: number): void {
        ...
        if (this.wbsToggleScrollAnchor) {
            ...
            if (group.yOrder === 0 && visualOffset <= 0 && Math.abs(visualOffset) <= this.taskElementHeight) {
                this.debugLog(`WBS top-row anchor restoration: group=${groupId}, forcing scrollTop=0`);
    ...
    private drawWbsGroupHeaders(
        ...
        const rowBorderColor = self.resolveColor("#D7DEE8", "foreground");
        const buttonFillColor = self.resolveColor("#FFFFFF", "background");
        const buttonStrokeColor = self.resolveColor("#C5CFDA", "foreground");
        const badgeTextColor = wbsTextColor;
        const defaultGroupBg = self.resolveColor(defaultGroupHeaderColor, "background");
        const mutedBadgeFill = self.highContrastMode
            ? buttonFillColor
            : self.blendColors(defaultGroupHeaderColor, "#FFFFFF", 0.68);
        const badgeFillColor = self.resolveColor(mutedBadgeFill, "background");
        const badgeStrokeColor = self.highContrastMode
            ? buttonStrokeColor
            : self.toRgba(rowBorderColor, 0.85);
        ...
        groupsUpdate.each(function (group) {
            ...
            const summaryFillColor = self.blendColors(groupSummaryColor, accentColor, 0.9);
            const summaryStrokeColor = self.toRgba(self.getSoftOutlineColor(summaryFillColor), group.isExpanded ? 0.55 : 0.82);
            const summaryCapColor = self.getSoftOutlineColor(summaryFillColor);
            const bgOpacity = self.highContrastMode ? 1 : ((group.visibleTaskCount === 0) ? 0.68 : 1);
            const summaryTextColor = groupNameColor;
            const groupNameWeight = group.level <= 1 ? '700' : '600';
            ...
            g.select<SVGRectElement>('.wbs-level-accent')
                ...
                .style('opacity', self.highContrastMode ? 1 : (group.visibleTaskCount === 0 ? 0.55 : 0.9));
            ...
            if (!showGroupName) {
                ...
                textElement
                    ...
                    .style('font-family', self.getFontFamily())
                    .style('font-weight', groupNameWeight)
    ...
    private renderLegend(viewportWidth: number): void {
        ...
        const textColor = this.getHeaderLegendTextColor();
        const borderColor = this.getHeaderLegendBorderColor();
        const dividerColor = this.highContrastMode ? this.getForegroundColor() : borderColor;
    ...
    private getSemanticTaskFillColor(
        ...
        applyCriticalFormat: boolean = true
    ): string {
        return getSemanticTaskFillColorForStyle(
            this.getCriticalFormatTask(task, applyCriticalFormat),
            this.getCriticalBarStyle(),
            this.legendDataExists,
            fallbackColor,
            criticalColor,
            nearCriticalColor
        );
    ...
    private getCriticalStatusMarkerStyle(
        ...
        return getCriticalStatusMarkerDescriptor({
            task: this.getCriticalFormatTask(task, applyCriticalFormat),
            style: this.getCriticalBarStyle(),
            criticalColor,
            nearCriticalColor,
            criticalThickness: this.settings.criticalPath.criticalBorderWidth.value,
            nearCriticalThickness: this.settings.criticalPath.nearCriticalBorderWidth.value,
            isMilestone
        });
    }

    private getCriticalStatusMarkerBatchKey(markerStyle: CriticalStatusMarkerDescriptor): string {
        return `${markerStyle.color}|${markerStyle.thickness}|${markerStyle.dashed ? "dashed" : "solid"}`;
    }
    ...
    private isTaskInLookAheadWindow(task: Task, lookAheadWindow: LookAheadWindow | null = this.getLookAheadWindow()): boolean {
        ...
        return geometry.segments.some(segment => {
            const startTime = this.normalizeToStartOfDay(segment.start);
            const finishTime = this.normalizeToStartOfDay(segment.finish);
    ...
    private getTaskRenderStyle(
        ...
        const criticalBarStyle = this.getCriticalBarStyle();
        const criticalFormatTask = this.getCriticalFormatTask(task, applyCriticalFormat);
        const useCriticalOutline = shouldUseCriticalOutline(criticalBarStyle, criticalFormatTask);
        ...
        if (useCriticalOutline && criticalFormatTask.isCritical) {
            ...
            hoverStrokeOpacity = 1;
        } else if (useCriticalOutline && criticalFormatTask.isNearCritical) {
    ...
    private getColorContrastRatio(foregroundColor: string, backgroundColor: string): number {
        const foreground = this.hexToRgb(foregroundColor);
        const background = this.hexToRgb(backgroundColor);
        const values = [foreground.r, foreground.g, foreground.b, background.r, background.g, background.b];
        if (values.some(value => Number.isNaN(value))) {
            return Number.POSITIVE_INFINITY;
        }

        const foregroundLuminance = this.getLuminance(foreground.r, foreground.g, foreground.b);
        const backgroundLuminance = this.getLuminance(background.r, background.g, background.b);
        const lighter = Math.max(foregroundLuminance, backgroundLuminance);
        const darker = Math.min(foregroundLuminance, backgroundLuminance);
        return (lighter + 0.05) / (darker + 0.05);
    }

    private getReadableTextColor(preferredTextColor: string, backgroundColor: string): string {
        const minimumContrastRatio = 4.5;
        if (this.getColorContrastRatio(preferredTextColor, backgroundColor) >= minimumContrastRatio) {
            return preferredTextColor;
        }

        const blackContrast = this.getColorContrastRatio("#000000", backgroundColor);
        const whiteContrast = this.getColorContrastRatio("#FFFFFF", backgroundColor);
    ...
    private applyHighContrastStyling(): void {
        ...
        this.legendContainer
            ...
            .style("border-top", `1px solid ${foreground}`)
            .style("box-shadow", "none");
    ...
    private buildTooltipDataItems(task: Task): VisualTooltipDataItem[] {
        ...
        const startText = this.formatDate(this.getTaskBarLabelStart(task));
        if (startText) items.push({ displayName: startLabel, value: startText });

        const finishText = this.formatDate(this.getTaskBarLabelFinish(task));
    ...
    private getWbsExportRowBackgroundColor(level: number, fallbackBackground: string): string {
        ...
        return this.highContrastMode
            ...
            : this.resolveColor(levelStyle.background, "background");
    }

    private getWbsExportRowTextColor(backgroundColor: string, fallbackText: string = "#333333"): string {
        if (this.highContrastMode) {
            return this.highContrastForeground;
        }

        const preferredTextColor = this.resolveColor(this.getWbsTextColor(fallbackText), "foreground");
        return this.getReadableTextColor(preferredTextColor, backgroundColor);
    }

    private getWbsExportCellStyle(backgroundColor: string, textColor: string, style: string): string {
        return `${style} background-color: ${backgroundColor}; color: ${textColor};`;
    }
